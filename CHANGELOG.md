...
[Keep a Changelog](https://keepachangelog.com/); the project is pre-1.0 and not yet
versioned, so changes accumulate under **Unreleased** until the first tagged release.
...
- **Recurrence** for Calendar events and Reminders — pass an RFC 5545 `RRULE` string
  ...
  (enforced at the boundary as a clear error); unsupported RRULE parts (e.g. `BYDAY`) are
...
- **`safari_open`** — open a URL in a new Safari tab (defaults the scheme to `https://`).
